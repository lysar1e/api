...
export class LinkService {
  constructor(
    @InjectRepository(Link) private linkRepository: typeof Link,
    @InjectRepository(Board) private boardRepository: typeof Board
  ) {}

  async generateLink({ boardId }: GenerateLinkDto) {
    const board = await this.boardRepository.findOne({
      where: { id: boardId },
    });
    const code = shortid.generate();
    const to = process.env.CLIENT_URL + "/invite/" + code;
    const generatedLink = await this.linkRepository
      .create({ code, board_id: boardId, to })
      .save();
  ...
  async joinBoard(dto: JoinBoardDto, contributor) {
    const { board_id } = await this.linkRepository.findOne({
      where: { code: dto.code },
    });
    if (!board_id) {
    ...
    if (
      board.owner === contributor ||
      board.contributors.includes(contributor)
    ) {
